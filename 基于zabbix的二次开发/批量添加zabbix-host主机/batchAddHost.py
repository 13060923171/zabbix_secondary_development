...
def comment():
    df = pd.read_excel('LFL China Equipment List.202100506.xls', sheet_name='Network List').loc[:,
    ...
    for i in range(len(df['City'])):
        ...
        elif 'Router' in df['Device Type'][i]:
            gruopname = 'LFL-DC-{}-{}-Network-Router'.format(df['City'][i], df['DC Name'][i])
            template = 'Templates-LFL-Cisco L3 SNMP'
            list_hostname.append(df['Hostname'][i])
            list_hostip.append(df['IP'][i])
            list_groupname.append(gruopname)
            list_template.append(template)
        elif 'Wireless' in df['Device Type'][i]:
            gruopname = 'LFL-DC-{}-{}-Network-Wireless'.format(df['City'][i], df['DC Name'][i])
            template = 'Templates-LFL-Cisco L2 SNMP'
            list_hostname.append(df['Hostname'][i])
            list_hostip.append(df['IP'][i])
            list_groupname.append(gruopname)
            list_template.append(template)
        elif 'Switch' in df['Device Type'][i] and 'C3' in df['Hostname'][i]:
            gruopname = 'LFL-DC-{}-{}-Network-SW'.format(df['City'][i], df['DC Name'][i])
            template = 'Templates-LFL-Cisco L3 SNMP'
            list_hostname.append(df['Hostname'][i])
            list_hostip.append(df['IP'][i])
